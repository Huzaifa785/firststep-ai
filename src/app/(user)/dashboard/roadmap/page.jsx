...
import { getRecommendations } from "@/utils/firebase/recommendations/write";
  ...
  <div className="relative">
    <div className="flex items-center gap-4">
      ...
      <div>
        ...
        <p className="text-sm font-medium text-violet-600 mt-1">{data.expectedPackage}</p>
      </div>
    </div>
    {!isLast && (
  ...
  <div className="bg-white p-4 rounded-xl border border-gray-200">
    ...
    <div className="space-y-2">
      ...
      <p className="text-sm text-gray-500">
        Proficiency: <span className="text-violet-600 font-medium">{proficiencyLevel}</span>
      </p>
    </div>
  </div>
);
...
const Roadmap = () => {
  ...
  // Always call useSelector, but only use its value after client-side hydration
  ...
  useEffect(() => {
    const fetchRecommendations = async () => {
      ...
      try {
        setIsLoading(true);
        const recommendations = await getRecommendations({ uid: user.uid });
        setRoadmapData(recommendations);
      } catch (err) {
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center space-y-4">
          ...
          <p className="text-gray-600">No recommendations available. Please complete the assessment first.</p>
        </div>
      </div>
    );
  }
      ...
      {career?.industryOutlook && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <StatsCard 
            icon={DollarSign} 
            label="Top Package" 
            value={career.industryOutlook.topRecruiters?.[0]?.averagePackage || 'N/A'}
          />
        </div>
      )}
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {additionalInsights?.careerProgression && (
            <div className="bg-white p-6 rounded-2xl border border-gray-200">
              <h2 className="text-lg font-semibold text-gray-900 mb-6">Career Progression</h2>
              <div className="space-y-8">
                {Object.entries(additionalInsights.careerProgression).map(([year, data], index, arr) => (
                  <TimelineItem 
                    key={year}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <SafeList 
                  ...
                  render={(course, idx) => <CourseCard key={idx} course={course} />}
                />
              </div>
            </div>
          )}
        ...
        <div className="space-y-4">
          ...
          {career?.industryOutlook && (
            <AccordionItem title="Industry Outlook" icon={Target}>
              <div className="space-y-4">
                <p className="text-gray-600">{career.industryOutlook.futureProspects}</p>
                <div className="space-y-4">
                  <SafeList 
                    items={career.industryOutlook.topRecruiters}
                    render={(recruiter, idx) => (
                      <div key={idx} className="bg-white p-4 rounded-lg border border-gray-200">
                        <h4 className="font-medium text-gray-900 mb-2">{recruiter.type}</h4>
                        <div className="space-y-2">
                          <div className="flex flex-wrap gap-2">
                            <SafeList 
                              items={recruiter.companies}
                              render={(company, idx) => (
                                <span key={idx} className="text-sm text-gray-600">{company}</span>
                              )}
                            />
                          </div>
                          <p className="text-sm text-violet-600 font-medium">{recruiter.averagePackage}</p>
                        </div>
                      </div>
                    )}
                  />
                ...
                <div className="bg-violet-50 p-4 rounded-lg">
                  ...
                  <ul className="space-y-2">
                    <SafeList 
                      ...
                      render={(tip, idx) => (
                        <li key={idx} className="flex items-center gap-2 text-sm text-violet-700">
            ...
            <AccordionItem title="Higher Education" icon={GraduationCap}>
              <div className="space-y-4">
                <SafeList 
                  ...
                  render={(option, idx) => (
                    <div key={idx} className="bg-white p-4 rounded-lg border border-gray-200">
                      <h4 className="font-medium text-gray-900 mb-2">{option.degree}</h4>
